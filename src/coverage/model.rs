    ...
    pub fn check(
        ...
        if let Some(user_weight) = HashCode::hash_code(&value.to_string(), feature_name, HashCodeVersion::from(hash_version)) {
            if let Some(range) = option_range {
                if range.in_range(&user_weight) { Some(FeatureResult::force(force_value.clone())) } else { None }
            } else if let Some(coverage) = option_coverage {
                if coverage.gt(&user_weight) { Some(FeatureResult::force(force_value.clone())) } else { None }
            } else {
                None
            }
        } else {
